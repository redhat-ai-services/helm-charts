...
As of RHOAI 3.3, the ODH Model Controller has the ability to automatically apply resources and tolerations directly from the hardware profile.

#### Setting the Hardware profile

The hardware profile can be configured using the following options:

```sh
helm upgrade -i [release-name] redhat-ai-services/vllm-kserve \
  --set 'hardwareProfile.name=my-hardware-profile'
```

The hardwareProfile defaults to using the `redhat-ods-applications` namespace, but if you wish to use a hardware profile in a different namespace you can specify it:

```sh
helm upgrade -i [release-name] redhat-ai-services/vllm-kserve \
  --set 'hardwareProfile.name=my-hardware-profile' \
  --set 'hardwareProfile.namespace=my-namespace'
```

Additionally, you manually set resources.
...
```sh
helm upgrade -i [release-name] redhat-ai-services/vllm-kserve \
  --set image.image=registry.redhat.io/rhaiis/vllm-rocm-rhel9 \
...
>[!IMPORTANT]
>The CPU only version of RHAIIS is Tech Preview.

```sh
helm upgrade -i [release-name] redhat-ai-services/vllm-kserve \
  --set image.image=registry.redhat.io/rhaiis/vllm-cpu-rhel9
```

Additional arguments and enviromnet vairabls may be required.
